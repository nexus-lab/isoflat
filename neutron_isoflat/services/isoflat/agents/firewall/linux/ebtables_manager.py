        ...
        try:
            ...
            for command in commands:
                ...
                if command.startswith('#') or command.startswith('\n'):
                    ...
                    continue
                self.execute(_args, run_as_root=True)
...
def _generate_path_between_rules(old_rules, new_rules):
    ...
    statements = [':%s' % c for c in sorted(new_chains - old_chains)]
    iso_chains = []
    other_chains = []
    for chain in sorted(old_chains | new_chains):
        if '-isoflat-' in chain:
            iso_chains.append(chain)
        else:
            other_chains.append(chain)

    for chain in other_chains + iso_chains:
